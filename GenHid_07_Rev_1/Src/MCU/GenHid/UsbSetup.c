...
const int8_ptr_t		gpStrRep = "This is comming from HID (9876543210)\0";	// 38 characters
...
{
	uint8_t		uDest = (uint8_t) (gpStp->bmRequestType & 0xF);
	uint16_t	uStat = 0;

	DBG_PRINTF("%s(): wIndex=0x%b\r\n", __FUNCTION__, gpStp->wIndex);
	//DBG_BREAK();
	...
	gpEp0Buf[0] = (uint8_t) (uStat >> 8);
	gpEp0Buf[1] = (uint8_t) (uStat & 0xFF);

	USB_EP_SET_DSQ_SYNC(USB_EP_REG0, SET);
	USB_EP_SET_MAX_PAYLOAD(USB_EP_REG0, 2);

#elif defined _CHIP_STM32F10XXXXX

	gpEp0Buf = (uint32_ptr_t) USB_EP_GET_TX_BUF_ADDR(USB_EP_REG_0, 0);
	MemCopyToPaddedBuffer(gpEp0Buf, &uStat, 2);
...
void On_Stp_Std_GetDesc_DevQlfy()
{
	...
	USB_EP_SET_STALL(USB_EP_REG0);
	//USB_EP_CLR_RDY_AND_SET_STALL(USB_EP_REG0);
	//USB_EP_SET_MAX_PAYLOAD(USB_EP_REG0, 0);
...
void On_Stp_Cls_Hid_GetReport()
{
	...
	MemCopy(gpEp0Buf, (uint8_ptr_t) gpStrRep, 37);

	USB_EP_SET_DSQ_SYNC(USB_EP_REG0, SET);
	USB_EP_SET_MAX_PAYLOAD(USB_EP_REG0, 37);

#elif defined _CHIP_STM32F10XXXXX

	gpEp0Buf = (uint32_ptr_t) USB_EP_GET_TX_BUF_ADDR(USB_EP_REG_0, 0);
	MemCopyToPaddedBuffer(gpEp0Buf, (uint16_ptr_t) gpStrRep, 37);

	USB_EP_SET_TX_BUF_LEN(USB_EP_REG_0, 0, 37);
	USB_EP_CTL(USB_EP_REG_0, USB_EP_CTL_CTRL_TRANS_SETUP_IN);
...
void On_Stp_Cls_Hid_GetIdle()
{
	...
	*gpEp0Buf = 0;
	...
	USB_EP_SET_TX_BUF_LEN(USB_EP_REG_0, 0, 1);
	USB_EP_CTL(USB_EP_REG_0, USB_EP_CTL_CTRL_TRANS_SETUP_IN);
...
void On_Stp_Cls_Hid_SetReport()
{
	...
	USB_EP_SET_DSQ_SYNC(USB_EP_REG1, SET);
	USB_EP_SET_MAX_PAYLOAD(USB_EP_REG1, gpStp->wLength);

#elif defined _CHIP_STM32F10XXXXX
	
	//DBG_PRINTF("%s(): Dir=%d, Ep0=0x%X\r\n", __FUNCTION__, USB_GET_TRANSACT_DIR(), IO_MMAP(USB_EP_REG_ADDR(USB_EP_REG_0)));
	USB_EP_CTL(USB_EP_REG_0, USB_EP_CTL_CTRL_TRANS_SETUP_OUT);
