...
extern uint32_t ClkGetSysClk();
extern uint32_t ClkGetAPB1Devider();
...
void UaInit()
{
	...
	uint32_t	uClk, uMant, uFract, uAPB1Dev;

	uClk = ClkGetSysClk();
	uAPB1Dev = ClkGetAPB1Devider();
	uClk = uClk / uAPB1Dev;
	uMant = uClk / 14745600;
	if (uMant > 0) {
	...
	USART_SET_BAUD_FRACT(UA_SEL_PORT, uMant, uFract);
