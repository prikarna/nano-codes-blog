...
void InitGPIO()
{
	...
	GPIO_SET_MODE_4(GPIO_PORT_A, GPIO_PIN_0, GPIO_MODE_INPUT);
	GPIO_SET_MODE_4(GPIO_PORT_A, GPIO_PIN_4, GPIO_MODE_OUTPUT);
	GPIO_ENA_INTR_4(GPIO_PORT_A, GPIO_PIN_0, GPIO_INTR_MODE_LEVEL_TRIG, GPIO_INTR_TYPE_HIGH, ENABLE);

#elif defined _CHIP_STM32F10XXXXX

	RCC_ENA_APB2_CTRL_CLK(ENABLE, RCC_APB2_CTRL_IOPB);
	GPIO_SET_MODE_INPUT4(GPIO_PORT_B, GPIO_PIN_6, GPIO_CNF_INP_PU_PD);
	GPIO_SET_MODE_OUTPUT4(GPIO_PORT_B, GPIO_PIN_12, GPIO_CNF_OUT_GEN_PUSH_PULL, GPIO_MOD_OUT_2MHZ);
	GPIO_WRITE4(GPIO_PORT_B, GPIO_PIN_12, 0xF);
...
void MainLoop()
{
	...
	while (1)
	{
		...
		if (uInts & USB_INT_ERROR)
		{
			On_ErrorInt();
		}
		else if (uInts & USB_INT_EXPECT_START_OF_FRAME)
		{
